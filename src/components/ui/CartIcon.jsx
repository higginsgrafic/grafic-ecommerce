import React from 'react';

function CartIcon({
  count = 0,
  onClick,
  className = '',
  iconSize = 'clamp(1.75rem, 3.5vw, 1.640625rem)',
  badgeSize = 'clamp(1.40625rem, 3.125vw, 1.23046875rem)',
  badgeFontSize = 'clamp(0.875rem, 1.875vw, 0.8203125rem)'
}) {
  return (
    <button
      onClick={onClick}
      className={`relative hover:bg-gray-50 rounded-md transition-colors ${className}`}
      aria-label="Afegir al cistell"
      style={{ padding: 'clamp(0.1rem, 0.35vw, 0.2rem)' }}
    >
      <img
        src={count > 0 ? "/cistell-ple-1.svg" : "/cistell-buit.svg"}
        alt={count > 0 ? "Cistell amb productes" : "Cistell"}
        width="28"
        height="28"
        className="transition-all duration-200"
        style={{ width: iconSize, height: iconSize }}
      />
      {count > 0 && (
        <div
          className="absolute pointer-events-none flex items-center justify-center rounded-full bg-black"
          style={{
            top: '-12px',
            right: '-15px',
            width: badgeSize,
            height: badgeSize
          }}
        >
          {/* Badge number */}
          <span
            className="font-bold font-oswald text-white"
            style={{
              fontSize: 'clamp(0.6875rem, 1.75vw, 0.67rem)',
              lineHeight: '1'
            }}
          >
            {count}
          </span>
        </div>
      )}
    </button>
  );
}

export default CartIcon;
