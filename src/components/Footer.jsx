...
const Footer = () => {
  ...
  const collections = [
    { name: texts.footer.collections.firstContact, path: '/first-contact', icon: '/logo-first-contact.png' },
    { name: texts.footer.collections.theHumanInside, path: '/the-human-inside', icon: '/logo-the-human-inside.png' },
    { name: texts.footer.collections.austen, path: '/austen', icon: '/logo-austen.svg' },
    { name: texts.footer.collections.outcasted, path: '/outcasted', icon: '/logo-outcasted.png' },
    { name: texts.footer.collections.cube, path: '/cube', icon: '/logo-cube.png' }
  ];
  ...
  useEffect(() => {
    ...
    const calculateMobileGap = () => {
      ...
      // Distribuir uniformement: 5 imatges + 4 gaps = 9 parts iguals
      const imageWidth = availableWidth / 9;
  ...
  return (
    <footer className="bg-white transition-colors duration-200">
              ...
              return (
                <React.Fragment key={index}>
                  <Link
                    ...
                    style={{ width: gap > 0 ? `${gap}px` : '110px' }}
                  >
                    {/* Imatge del coet (visible sempre a 100% opacitat, s'ajusta a l'amplada del contenidor) */}
                    <img
                      src={collection.icon}
                      alt={collection.name}
                      className="transition-transform duration-300 group-hover:scale-110 w-full h-auto object-contain"
                      style={{
                        display: 'block',
                        opacity: 1,
      ...
      <div className="lg:hidden bg-gray-50 transition-colors duration-200">
        <div ref={mobileContainerRef} className="max-w-7xl mx-auto px-4 py-12 min-h-[100px]">
          {/* Imatges de col·leccions en dues files - CENTRADES EN MÒBIL */}
          <motion.div
          ...
          >
            {collections.map((collection, index) => (
              <motion.div
                ...
                className={`flex items-center justify-center ${index >= 3 ? 'col-start-2' : ''}`}
              >
                <Link
                  to={collection.path}
                  className="flex items-center justify-center transition-transform hover:scale-110 active:scale-95 group w-16"
                  title={collection.name}
                >
                  <img
                    src={collection.icon}
      ...
      <div className="bg-white transition-colors duration-200">
        <div className="max-w-7xl mx-auto py-[82px] lg:py-[98px] px-4 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-12 lg:gap-x-12">
            {/* Botiga */}
            <div className="text-left">
              ...
              <ul className="space-y-2.5 lg:space-y-3 text-left">
                <li><Link to="/collections" className="font-roboto text-[12pt] font-normal transition-all inline-block text-gray-900" onMouseEnter={(e) => { const color = document.documentElement.classList.contains('dark') ? '#ffffff' : '#141414'; e.target.style.textShadow = `0 0 0.55px ${color}, 0 0 0.55px ${color}`; }} onMouseLeave={(e) => e.target.style.textShadow = 'none'}>{texts.footer.services.shop.collections}</Link></li>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          >
            <Link to="/" className="flex items-center justify-center flex-shrink-0 transition-transform hover:scale-105 active:scale-95">
              <img src="/logo-grafc.png" alt="GRAFC" className="h-[50px] w-auto block" loading="lazy" />
            </Link>
          </motion.div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
          <div className="flex items-center justify-center min-h-[80px]">
            <Link to="/" className="transition-transform hover:scale-105">
              <img src="/logo-grafc.png" alt="GRAFC" className="h-[66px] w-auto block" loading="lazy" />
            </Link>
          </div>
        </div>
      </div>
