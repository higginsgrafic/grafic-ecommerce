...
export const wordpressAPI = {
  /**
   * Obtenir textos editats
   */
  getTexts: async () => {
    if (USE_MOCK) {
      const { siteTexts } = await import('@/data/siteTexts');
      return siteTexts;
    }
    return apiClient.get('/wordpress/texts');
  },

  /**
   * Desar textos editats
   */
  saveTexts: async (texts) => {
    if (USE_MOCK) {
      return { success: true };
    }
    return apiClient.post('/wordpress/texts', texts);
  },

  /**
   * Obtenir estils editats
   */
  getStyles: async () => {
    if (USE_MOCK) {
      const { siteStyles } = await import('@/data/siteStyles');
      return siteStyles;
    }
    return apiClient.get('/wordpress/styles');
  },

  /**
   * Desar estils editats
   */
  saveStyles: async (styles) => {
    if (USE_MOCK) {
      return { success: true };
    }
    return apiClient.post('/wordpress/styles', styles);
  }
};
